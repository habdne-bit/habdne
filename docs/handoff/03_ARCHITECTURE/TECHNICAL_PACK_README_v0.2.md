> **v0.2.3 technical patch:** use `schema_v0.2.3.sql`, `seed_master_data_v0.2.3.sql`, and `openapi_v0.2.3.yaml`. The patch preserves v0.2.1 FK fixes and v0.2.2 D1/D2 semantics; it corrects only D6 schema-version metadata/packaging consistency. See `TECHNICAL_PATCH_v0.2.3.md`. Full runtime/Slice 0 rerun is required before Slice 1.

> **Handoff layout note:** this is the v0.2 technical README copied into the final developer handoff. Files have been reorganized into numbered folders. Use the root `README.md` and `00_START_HERE/TURAB_Developer_Handoff_Master_v1.0.3.md` for actual paths and authority order.
...
| File | Purpose |
|---|---|
| `schema_v0.2.3.sql` | Clean PostgreSQL 16+ baseline schema with v0.2 invariants |
| `seed_master_data_v0.2.3.sql` | Adrar location master data, controlled document/right options, reason codes, matching policy |
| `openapi_v0.2.3.yaml` | OpenAPI 3.1 contract with DTO separation, operationIds, idempotency and queues |
...
| `technical_pack_static_audit_v0.2.3.py` | Repeatable local static contract audit |
| `STATIC_AUDIT_RESULTS_v0.2.3.json` | Latest static audit result and file hashes |
...
```bash
python technical_pack_static_audit_v0.2.3.py
```
