#!/usr/bin/env python3
"""Run the REQUEST freshness pass, once, against the active policy.
...
from turab.services import freshness, requests as request_service  # noqa: E402
...
def main() -> int:
    ...
    try:
        ...
        with Session(bind=engine, future=True) as reader:
            try:
                days, policy = freshness.request_threshold_days(reader)
            except freshness.NoActiveFreshnessPolicy as exc:
                print(f"refusing to run: {exc}", file=sys.stderr)
                return 2
            print(f"policy {policy}: request freshness window is {days} days")

            if args.dry_run:
                candidates = request_service.stale_active_requests(
                    reader, limit=args.limit
                )
                print(f"would move {len(candidates)} request(s)")
        ...
        with Session(bind=engine, future=True) as writer:
            ...
            with audited_transaction(writer, None, require_actor=False):
                moved = request_service.mark_stale_as_needing_confirmation(
                    writer, limit=args.limit
                )
        print(f"moved {len(moved)} request(s) to NEEDS_CONFIRMATION")
        for request_id in moved:
            print(f"  {request_id}")
        return 0
    finally:
        engine.dispose()
