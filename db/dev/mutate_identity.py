...
MUTATIONS = [
 ...
 ("I24 canonical-is-alias not named", SVC,
  '        if decision == "CONFIRMED_SAME" and canonical in aliased:', "        if False:"),
 ("I25 a pair member that is an alias not refused", SVC,
  "    if aliased:\n", "    if False:\n"),
 ("I26 alias-is-canonical not pre-checked", SVC,
  "        if acts_as_canonical:", "        if False:"),
 # review of c3aac8a
 ("I35 review checks only CONFIRMED_SAME (the defect reported)", SVC,
  "    _lock_pair_and_check(session, pair=pair, decision=decision,",
  "    if decision == \"CONFIRMED_SAME\": _lock_pair_and_check(session, pair=pair, decision=decision,"),
 ("I36 generation does not lock the involved properties", SVC,
  "ORDER BY property_id FOR SHARE", "ORDER BY property_id"),
 ("I37 generation does not re-check aliases after its lock", SVC,
  "            if a not in aliased and b not in aliased and still_blocked(a, b)]",
  "            if still_blocked(a, b)]"),
 ("I38 a focus that became an alias is not refused after the lock", SVC,
  "    if focus is not None and focus in aliased:", "    if False:"),
 # review of 1935dc1
 ("I39 blocking not re-checked after the lock", SVC,
  "            if a not in aliased and b not in aliased and still_blocked(a, b)]",
  "            if a not in aliased and b not in aliased]"),
 ("I40 generation locks FOR KEY SHARE (the mode before this review)", SVC,
  "ORDER BY property_id FOR SHARE", "ORDER BY property_id FOR KEY SHARE"),
