...
RULES: tuple[Rule, ...] = (
    ...
    Rule("R4.1", "PROPERTY authority: creator account OR a recorded claim",
         ("test_claim_event_grants_property_access",
          ...
          # Slice 3 step 8, §5.1 over HTTP: S10, S15
    ...
    Rule("R4.7 / S16", "A relation only DECLARED does not block a claimant",
    ...
    # --- Slice 3 steps 6 and 7 ----------------------------------------------
    Rule("R10.4 / step 6", "The public list needs no credential and reads none",
         ("test_no_credential_is_needed_or_read",
          "test_public_reads_refuse_an_operation_that_is_not_public")),
    Rule("R6.3 / step 6", "The public list records no per-request access audit",
         ("test_the_public_list_records_no_access_audit",)),
    Rule("§2.1 / step 7", "Identity decisions: REVIEWER or ADMIN, never OPERATOR",
         ("test_an_operator_cannot_review",
          "test_an_operator_generates_and_a_reviewer_cannot")),
    # --- INV-1 -------------------------------------------------------------
    ...
    Rule("R6.3c / Q4", "A list is audited once, never per row",
         ("test_a_list_is_audited_once_not_per_row", "test_list_endpoint_audits_once",
          # Slice 3: the property review queue (step 8)
          "test_the_queue_is_audited_once_and_refused_to_customers")),
    ...
    # --- independent review R-S2-01 .. R-S2-05 ------------------------------
    Rule("R-S2-01",
         "The version guard locks the row, so two patches holding the same "
         "version yield one success and one 409 on real PostgreSQL",
         ("test_two_patches_holding_the_same_version_yield_one_success_and_one_409",
          "test_the_refused_patch_leaves_the_value_it_tried_to_write_absent")),
    Rule("R-S2-02",
         "State and staleness decisions hold their conditions at write time, "
         "under two connections",
         ("test_reconfirm_racing_a_close_does_not_reopen_the_closed_request",
          "test_two_transitions_from_the_same_state_do_not_both_apply",
          "test_the_staleness_pass_does_not_mark_a_concurrently_reconfirmed_request",
          "test_the_staleness_pass_does_not_reopen_a_concurrently_closed_request")),
    Rule("R-S2-03 follow-up",
         "The slot collision is a typed 409 in BOTH branches, under a parent "
         "lock, with no partial change and no key consumed",
         ("test_moving_a_criterion_onto_an_occupied_slot_is_409",
          "test_the_refused_move_records_no_provenance_and_frees_its_key",
          "test_a_move_to_a_free_slot_still_succeeds",
          "test_two_concurrent_adds_of_the_same_slot_yield_one_success_and_one_409",
          "test_a_concurrent_move_onto_a_slot_being_taken_is_refused")),
    Rule("R-S2-05 follow-up",
         "An empty string is not a null, and a datetime without an offset is "
         "a 4xx rather than a 500",
         ("test_an_empty_free_text_field_is_accepted",
          "test_null_clears_a_nullable_field",
          "test_omitting_a_field_leaves_it_unchanged",
          "test_null_is_still_refused_for_a_non_nullable_field",
    ...
    Rule("Concurrency evidence",
         ...
         ("test_an_unexpected_worker_error_fails_the_assertions",)),
    Rule("R-S2-03",
         "The criteria endpoint adds AND changes, by the contract's declared id",
         ("test_the_contracts_criterion_id_is_accepted",
          "test_changing_a_criterion_does_not_duplicate_it",
          "test_changing_a_criterion_bumps_the_request_version",
          "test_a_criterion_id_from_another_request_is_refused",
          "test_an_unknown_criterion_id_is_refused",
          "test_re_adding_the_same_slot_points_at_the_change_path",
          "test_the_change_is_recorded_with_its_previous_value")),
    Rule("R-S2-04",
         "Reactivation applies the active freshness policy; a future "
         "confirmation is refused and a historical one accepted",
         ("test_a_stale_paused_request_cannot_be_reactivated",
          "test_a_never_confirmed_paused_request_cannot_be_reactivated",
          "test_reconfirm_then_reactivate_is_the_two_step_path",
          "test_the_freshness_check_on_reactivation_reads_the_active_policy",
          "test_a_future_confirmation_is_refused",
          "test_a_historical_confirmation_is_still_accepted")),
    Rule("R-S2-05",
         "Input models match the contract; expected errors are 4xx, write "
         "nothing, consume no key and leak no database text",
         ("test_a_null_non_nullable_field_is_a_422_not_a_500",
          "test_an_unknown_enum_value_is_a_422_not_a_500",
          "test_an_incoherent_budget_pair_is_refused_on_create",
          "test_the_budget_pair_is_checked_on_the_merged_values",
          "test_a_refused_input_consumes_no_idempotency_key",
          "test_a_refused_input_leaks_no_database_text")),
